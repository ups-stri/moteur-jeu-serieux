package scenariofactory
{
	...
	public dynamic class Scenario extends MovieClip {
		...
		public function _bringToFront(e:MouseEvent):void {
			...
			//   fils de Scenario (ex. du widget "telephone" dans le ssi.fla)
			// - instance d'un widget si c'est un sous-widget qui est cliqué
			//   (ex. du sous-widget "repertoire" de ce widget "telephone")
			var widgetClic:Widget = Widget(e.currentTarget);
			var clipParent:MovieClip = MovieClip(widgetClic.parent);
			trace("clic on "+e.currentTarget.name+", numChildren="+clipParent.numChildren);
			// changement d'index de n° d'enfant de widgetClic pour clipParent
			// (au plus haut : nombre de sesenfants - 1, puisque ça commmence à 0
			//  et que le dernier enfant est en avant-plan)
			clipParent.setChildIndex(widgetClic, clipParent.numChildren - 1);
        }
